...
def create_datasets(datasets, N):
    ...
    for i in range(N, len(datasets)):
        X.append(datasets.iloc[i - N:i][['lng', 'lat','Course','Speed']].values)
        y.append(datasets.iloc[i][['lng', 'lat','Course','Speed']].values)
    return torch.Tensor(np.array(X)).float().to(device), torch.Tensor(np.array(y)).float().to(device)

scaler = MinMaxScaler()
datasets[['lng', 'lat','Course','Speed']] = scaler.fit_transform(datasets[['lng', 'lat','Course','Speed']])
